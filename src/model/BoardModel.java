...
public class BoardModel implements Observable {
	 ...
	 * @modifies  occupied, multidimensional array of booleans. gizmos, arraylist of Gizmo objects.
	 **/
	private BoardModel(){
	...
	/**
	 * @effects   initializes "balls", "players", "cezmis", "engel", "walls", "state", "currentPlayer, "observers" and "cezerye" fields.
	 * @modifies  balls, an arraylist of Ball objects. players, an arraylist of Player objects. cezmis, an arraylist of Cezmi objects.
	 *	          engel, an Engel object. walls, an arraylist of Wall objects. state, an Enum. currentPlayer, a Player object. observers,
	 *            an arraylist of Observer objects. cezerye, a Cezerye object. 
	 **/
	...
	/**
	 * @effects   If boardModel is null, creates new BoardModel and returns it. Else it returns boardModel directly.
	...
	/**
	 * @effects    If level is LEVEL1, it creates a ball which is located at 5L above the left Cezmi and adds it to an
	 *	           arraylist named "balls". If level is LEVEL2, it creates two balls, one located at 5L above the left
	 *	           Cezmi and other located at 5L above the right Cezmi, then it adds these ball objectes to an arraylist
	 *	           named "balls".
	...
	/**
	 * @effects   Creates two Player objects one with id=0 and score=0, and other one with id=1 and score=0. And adds these Player
	 *	          objects to an arraylist named "players".
	...
	/**
	 * @effects   Creates two Cezmi objects one at x=5L and y=25L, and other one at x=15L and y=25L. Then sets the first cezmi's owner
	 *	          as the first player and sets the second cezmi's owner as the second player. Then adds these Cezmi objects to an 
	 *	          arraylist named "cezmis". 
	 *	          objects to an arraylist named "cezmis".
	 * @modifies  players, an ArrayList of Player objects. 
	 **/
	...
	/**
	 * @effects   Creates an Engel object at x=12.375L and y=22L and sets engel field to this object.
	...
	/**
	 * @effects   Creates 4 walls, one located at the top side of the board, one located at the right side of the board,
	 *            one located at the bottom side of the board and one located at the left side of the board. Then adds 
	 *	          these walls to an arraylist named "walls".
	...
	/**
	 * @effects   Initializes a multidimensional array of booleans named "occupied". Then it sets all booleans to "false".
	...
	/**
	 * @effects   Initializes "observers" which is an arrayList of Observer objects and then adds a winPanel Observer to this ArrayList.
	...
	/**
	 * @effects   Finds a random and empty location for placing a cezerye. Then places a cezerye object to this place.
	 * @modifies  cezerye, which is a Cezerye object. 
	 **/
	private void initializeCezerye() {
		int[] coordinates = randomAvailableCoordinates();
		cezerye = (Cezerye)BoardObjectFactory.getInstance().createBoardObject(BoardObjectEnum.CEZERYE, coordinates[0], coordinates[1]);
	}
	...
	/**
	 * @effects  Finds a random location and if this location is empty, it returns this location. Else it finds another random 
	 *	         location until this location is empty. 
	...
	public int[] randomAvailableCoordinates() {
		...
		int x = coordinates[0];
		int y = coordinates[1];
		if (occupied[x][y]) return randomAvailableCoordinates();
	...
	/**
	 * @requires  0<=x<=500, 0<=y<=500 and type is an enum which is equal to one of "BALL, CEZMI, ENGEL, CEZERYE, FIRILDAK,
	 *            TRIANGLE_TAKOZ, SQUARE_TAKOZ, LEFT_TOKAT, RIGHT_TOKAT, WALL".
	 * @effects   Finds the grid corresponding to the coordinates of the mouse click. Creates a Gizmo object at this grid. 
	 *	          adds this Gizmo object to an arraylist named gizmos. Sets the coordinates of this grid at the occupied
	 *	          matrix to "true". Checks if the size of the "gizmos" arraylist is 4, if it is, it sets the currentPlayer 
	 *	          as the second player.
	 * @modifies  gizmos, an ArrayList of Gizmo objects. occupied, a multidimensional array of booleans. currentPlayer, a Player object. 
	 * @param     x: X coordinate of the gizmo that will be added.
	 * @param     y: Y coordinate of the gizmo that will be added.
	 * @param     type
	 **/
	 ...
	 * @param     x: X coordinate of the gizmo that will be rotated.
	 * @param     y: Y coordinate of the gizmo that will be rotated.
	 **/
	...
	private Gizmo findGizmoAtGivenCoordinates(int x, int y) {
		for (int i=0; i<gizmos.size(); i++) {
			Gizmo gizmo = gizmos.get(i);
			if (gizmo.getX()==x && gizmo.getY()==y) return gizmo;
		}
		return null;
	}

	/**
	 * @effects   Finds the gizmo at coordinates (x,y) and deletes it. Also sets the element at xth row and yth column of
	 *            the occupied matrix to false.
	 * @modifies  occupied, a multidimensional array of booleans.
	 * @requires  0<=x<=500, 0<=y<=500
	 * @param     x: X coordinate of the gizmo that will be deleted.
	 * @param     y: Y coordinate of the gizmo that will be deleted.
	 **/
	...
	/**
	 * @effects   Finds the gizmo at coordinates (oldX,oldY) and moves it to (newX, newY). Also sets the element at 
	 *            (oldX)th row and (oldY)th column of occupied matrix to false and sets the element at (oldX)th row
	 *            and (oldY)th column of occupied matrix to true.
	 * @modifies  occupied, a multidimensional array of booleans.
	 * @requires  0<=oldX<=500, 0<=oldY<=500, 0<=newX<=500, 0<=newY<=500
	 * @param     oldX: Old X coordinate of the chosen Gizmo.
	 * @param     oldY: Old Y coordinate of the chosen Gizmo.
	 * @param     newX: New X coordinate of the chosen Gizmo.
	 * @param     newY: New Y coordinate of the chosen Gizmo.
	 **/
	...
	/**
	 * @effects   If cezmiIndex is 0, it moves the Cezmi at the left side of the board in the given direction. If it's 1,
	 *            it moves the Cezmi at the right side of the board in the given direction.
	 * @requires  0<=cezmiIndex<=1 and direction is DirectionEnum.LEFT or DirectionEnum.RIGHT           
	 * @param     cezmiIndex: is equal to 0 for the first Cezmi and 1 for the second Cezmi. 
	 * @param     direction: Direction that the Cezmi will go to.
	 **/
	public void moveCezmi(int cezmiIndex, DirectionEnum direction) {
		Cezmi cezmi = cezmis.get(cezmiIndex);
		if (direction == DirectionEnum.WEST) cezmi.moveLeft();
		else cezmi.moveRight();
	}
	...
	/**
	 * @effects   Sets balls field, which is an arraylist of Ball objects, to balls input.
	...
	/**
	 * @effects   Sets cezmis field, which is an arraylist of Cezmi objects, to cezmis input.
	...
	/**
	 * @effects   Sets walls field, which is an arraylist of Wall objects, to walls input.
	...
	/**
	 * @effects   Sets gizmos field, which is an arraylist of Gizmo objects, to gizmos input.
	...
	/**
	 * @effects   Sets players field, which is an arraylist of Player objects, to players input.
	 * @modifies  players, an arraylist of Player objects.
	 * @param     players
	 **/
	...
	/**
	 * @effects   Sets occupied field, which is a multidimensional array of booleans, to occupied input.
	 * @modifies  occupied, a multidimensional array of booleans. 
	 * @param     occupied
	 **/
	...
	/**
	 * @effects   Sets observers field, which is an arraylist of Observer objects, to observers input.
	 * @modifies  level, a Level object.
	 * @param     observers
	 **/
