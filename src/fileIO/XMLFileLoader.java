...
public class XMLFileLoader implements FileLoader {
	...
	/**
	 * @requires file to be a valid XML file. This validation is done by XMLFileValidator.
	 * @param file: The file which will be loaded.
	...
	public String loadFile(File file) {
		...
		try {
			...
			if(BoardModel.getInstance().getPlayers().get(0).getGizmoNum() < 4 && BoardModel.getInstance().getPlayers().get(1).getGizmoNum() == 4) {
				result += "\nPlayer1 needs to add " + (4-BoardModel.getInstance().getPlayers().get(0).getGizmoNum()) + " more gizmos.";
			}
			else if (BoardModel.getInstance().getPlayers().get(0).getGizmoNum() == 4 && BoardModel.getInstance().getPlayers().get(1).getGizmoNum() < 4){
					result += "\nPlayer2 needs to add " + (4-BoardModel.getInstance().getPlayers().get(1).getGizmoNum()) + " more gizmos.";
			}
			else if (BoardModel.getInstance().getPlayers().get(0).getGizmoNum() < 4	&& BoardModel.getInstance().getPlayers().get(1).getGizmoNum() < 4) {
						result += "\nPlayer1 needs to add "
								+ (4-BoardModel.getInstance().getPlayers().get(0).getGizmoNum()) + " more gizmos."
	...
	/**
	 * @effects: parses the level from the XML file and sets "level" field according to the data it parsed. 
	 * @modifies: level
	 * @param doc
	 */
	...
	/**
	 * @effects: parses the ball locations and ball velocities from the XML file and sets "balls" field according to
	 *           the data it parsed. 
	 * @modifies: level
	 * @param doc
	 */
	private boolean parseBalls(Document doc) {
		...
		for (int i = 0; i < ballList.getLength(); i++) {
			...
			if (nNode.getNodeType() == Node.ELEMENT_NODE) {
				...
				balls.add((Ball) BoardObjectFactory.getInstance().createBoardObject(BoardObjectEnum.BALL, ballX, ballY));
	...
	private Player findOwner(BoardObject boardObject) {
		if(boardObject.getX() >= 0 && boardObject.getX() <= 11 && boardObject.getY() >= 0 && boardObject.getY() <= 18) {
			BoardModel.getInstance().setCurrentPlayer(BoardModel.getInstance().getPlayers().get(0));
			return players.get(0);
		} else if (boardObject.getX() >= 13 && boardObject.getX() <= 24 && boardObject.getY() >= 0 && boardObject.getY() <= 18) {
	...
	/**
	 * @effects: parses the x coordinate of the first cezmi and the score of the first player from the XML file and sets "cezmi1"
	...
	private boolean parseCezmi(Document doc, int index) {
		...
		if (cezmi1List != null && cezmi1List.getLength() != 0) {
			...
			if (nNode.getNodeType() == Node.ELEMENT_NODE) {
				...
				if(index == 1){
					if (cezmiX<BoardModel.getInstance().getWalls().get(3).getX()+BoardModel.getInstance().getWalls().get(3).getWidth()
					 || cezmiX>BoardModel.getInstance().getEngel().getX()) {
						result = false;
					}
				}else  {
					if ((cezmiX<BoardModel.getInstance().getEngel().getX()+BoardModel.getInstance().getEngel().getWidth())
				     || (cezmiX>BoardModel.getInstance().getWalls().get(1).getX())) {
						result = false;
					}
				}
				cezmis.add((Cezmi) BoardObjectFactory.getInstance().createBoardObject(BoardObjectEnum.CEZMI, cezmiX, (double)(cezmiY/BoardModel.getL())));
	...
	/**
	 * @effects: parses the x, y coordinates and time field of the cezerye from the XML file
	 *           and sets "cezerye" according to the data it parsed. 
	 * @modifies: cezerye
	 * @param doc
	 */
	private boolean parseCezerye(Document doc) {
		...
		if (cezeryeList != null && cezeryeList.getLength() != 0) {
			...
			if (nNode.getNodeType() == Node.ELEMENT_NODE) {
				...
				cezerye = (Cezerye)BoardObjectFactory.getInstance().createBoardObject(BoardObjectEnum.CEZERYE, x, y);
	...
	/**
	 * @effects: parses the x, y coordinates of the squareTakozs from the XML file
	 *           and modifies gizmos according to the data it parsed. 
	 * @modifies: gizmos
	 * @param doc
	 */
		...
		if (squareTakozList != null && squareTakozList.getLength() != 0) {
			for (int i = 0; i < squareTakozList.getLength(); i++) {
				...
				if (nNode.getNodeType() == Node.ELEMENT_NODE) {
					...
					int x = Integer.parseInt(eElement.getAttribute("x"));
					int y = Integer.parseInt(eElement.getAttribute("y"));
					SquareTakoz squareTakoz = (SquareTakoz) BoardObjectFactory.getInstance().createGizmo(BoardObjectEnum.SQUARE_TAKOZ, x, y);
	...
	/**
	 * @effects: parses the x, y coordinates and the orientation field of the triangleTakozs from the XML file
	 *           and modifies gizmos according to the data it parsed. 
	 * @modifies: gizmos
	 * @param doc
	 */
		...
		if (triangleTakozList != null && triangleTakozList.getLength() != 0) {
			...
			for (int i = 0; i < triangleTakozList.getLength(); i++) {
				...
				if (nNode.getNodeType() == Node.ELEMENT_NODE) {
					...
					TriangleTakoz triangleTakoz = (TriangleTakoz) BoardObjectFactory.getInstance().createGizmo(BoardObjectEnum.TRIANGLE_TAKOZ, x, y);
					...
					if (orientation==0) {
						if (BoardModel.getInstance().addGizmo(x, y-1, BoardObjectEnum.TRIANGLE_TAKOZ, owner)) {
							for (int j = 0; j < rotationCount; j++) {
								((TriangleTakoz)BoardModel.getInstance().getGizmos().get(BoardModel.getInstance().getGizmos().size()-1)).startRotating();
							}
						}
						else {
							//TODO
	...
	/**
	 * @effects: parses the x, y coordinates and the angle field of the firildaks from the XML file
	 *           and modifies gizmos according to the data it parsed. 
	 * @modifies: gizmos
	 * @param doc
	 */
		...
		if (firildakList != null && firildakList.getLength() != 0) {
			for (int i = 0; i < firildakList.getLength(); i++) {
				...
				if (nNode.getNodeType() == Node.ELEMENT_NODE) {
					...
					Firildak firildak = (Firildak)BoardObjectFactory.getInstance().createGizmo(BoardObjectEnum.FIRILDAK, x, y);
	...
	/**
	 * @effects: parses the x, y coordinates and the orientation field of the leftTokats from the XML file
	 *           and modifies gizmos according to the data it parsed. 
	 * @modifies: gizmos
	 * @param doc
	 */
		...
		if (leftTokatList!= null && leftTokatList.getLength() != 0) {
			for (int i = 0; i < leftTokatList.getLength(); i++) {
				...
				if (nNode.getNodeType() == Node.ELEMENT_NODE) {
					...
					int y = Integer.parseInt(eElement.getAttribute("y"));
					int orientation = Integer.parseInt(eElement.getAttribute("orientation"));
					LeftTokat leftTokat = (LeftTokat)BoardObjectFactory.getInstance().createGizmo(BoardObjectEnum.LEFT_TOKAT, x, y);
	...
	/**
	 * @effects: parses the x, y coordinates and the orientation field of the rightTokats from the XML file
	 *           and modifies gizmos according to the data it parsed. 
	 * @modifies: gizmos
	 * @param doc
	 */
		...
		if (rightTokatList != null && rightTokatList.getLength() != 0) {
			for (int i = 0; i < rightTokatList.getLength(); i++) {
				...
				if (nNode.getNodeType() == Node.ELEMENT_NODE) {
					...
					int y = Integer.parseInt(eElement.getAttribute("y"));
					int orientation = Integer.parseInt(eElement.getAttribute("orientation"));
					RightTokat rightTokat = (RightTokat)BoardObjectFactory.getInstance().createGizmo(BoardObjectEnum.RIGHT_TOKAT, x, y);
	...
	/**
	 * @effects: parses the keys that the players will use during the game from the XML file
	 *           and modifies players according to the data it parsed. 
	 * @modifies: players
	 * @param doc
	 */
